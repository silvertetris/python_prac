...
def remove_repetitive_sentences(segments, similarity_threshold=0.9):
    """
    코사인 유사도, TF-IDF, 형태소 기반 벡터 거리 계산을 활용하여 중복 및 버벅거림 제거
    """
    filtered_segments = []
    prev_embedding = None
    ...
    for i, segment in enumerate(segments):
        ...
        # 비교 로직
        if not (is_similar_cosine or is_similar_tfidf or is_close_euclidean):
            # 이전 문장과 충분히 다르면 추가
...
# 6. JSON 파일 불러오기
# ======================================
input_file_path = "/content/drive/MyDrive/STT3_segments.json"
